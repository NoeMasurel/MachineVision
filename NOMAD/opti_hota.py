...
to maximize HOTA, using NOMAD (PyNomad). ID-count error is recorded per eval
...
from TrackEval.Eval import hota_score
...
def metrics_for_point(confidence: float, tracker_overrides: dict):
    ...
    _hota, summary = hota_score(str(gt_path), OCCLUDED, str(pred_file), return_summary=True)  # type: ignore
    if "HOTA.HOTA" not in summary:
        raise KeyError(f"No 'HOTA.HOTA' key in summary. Available keys: {list(summary.keys())}")
    ...
    return summary["HOTA.HOTA"], id_diff, config_path
...
class OptimizationState:
    ...
    def __init__(self):
        self.all_evals = []
        self.best_evals_by_hota = []
        self.best_evals_by_id_diff = []
        self.recent_hotas = deque(maxlen=STAGNATION_WINDOW)
    ...
    def record_success(self, eval_record: dict) -> bool:
        ...
        with self.lock:
            ...
            _update_top_n(self.best_evals_by_hota, eval_record, "hota", reverse=True, n=N_BEST)
            _update_top_n(self.best_evals_by_id_diff, eval_record, "id_diff", reverse=False, n=N_BEST)

            self.recent_hotas.append(eval_record["hota"])
    ...
    def _is_stagnated(self) -> bool:
        if len(self.recent_hotas) < STAGNATION_WINDOW:
            return False
        window_max, window_min = max(self.recent_hotas), min(self.recent_hotas)
    ...
    def _save_partial(self):
        ...
        payload = {
            ...
            "best_evals_by_hota": self.best_evals_by_hota,
...
def make_blackbox(state: OptimizationState):
    ...
    def bb(x):
        ...
        try:
            hota, id_diff, config_path = metrics_for_point(confidence, tracker_overrides)
        ...
        print(f"[OK] {label}  HOTA={hota:.4f}  ID_diff={id_diff}  config={config_path}")
        eval_record.update({
            "success": True,
            "hota": hota,
            "id_diff": id_diff,
...
def _format_eval_line(eval_record: dict) -> str:
    ...
    return (f"{values}  HOTA={eval_record['hota']:.4f}  ID_diff={eval_record['id_diff']}  "
            f"config={eval_record.get('tracker_config_yaml')}")


def print_best(state: OptimizationState):
    print(f"\n--- Top {N_BEST} by HOTA (highest first) ---")
    if not state.best_evals_by_hota:
        print("No successful evaluation found -- every trial failed.")
    else:
        for i, ev in enumerate(state.best_evals_by_hota, start=1):
