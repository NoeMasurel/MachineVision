...
def metrics_for_point(confidence: float, tracker_overrides: dict):
    ...
    hota, summary = hota_score(str(gt_path), OCCLUDED, str(pred_file), return_summary=True)  # type: ignore
...
class OptimizationState:
    ...
    def record_success(self, eval_record: dict) -> bool:
        ...
        with self.lock:
            ...
            if self.best_eval is None or eval_record["hota"] > self.best_eval["hota"]:
...
def print_best(state: OptimizationState):
    ...
    print(f"{values}  HOTA={state.best_eval['hota']:.4f}  ID_diff={state.best_eval['id_diff']}  "
